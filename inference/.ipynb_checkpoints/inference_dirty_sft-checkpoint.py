...
def test_model(chatbot: ElderlySFTChatbot, output_file: str):
    """批量测试SFT模型效果"""
    test_questions = [
        "我最近总是忘记事情，这是老年痴呆吗？",
        "如何保持身体健康？",
...
def interactive_chat(chatbot: ElderlySFTChatbot):
    """SFT模型交互式对话"""
    print("\n" + "="*80 + "\n🎯 老年人关怀助手交互式对话\n" + "="*80)
...
def main():
    parser = argparse.ArgumentParser(description="老年人关怀SFT模型推理脚本")
    ...
    parser.add_argument("--test_output_file", type=str, default="elderly_sft_test_results.json", help="批量测试结果的输出文件路径")
    ...
    chatbot = ElderlySFTChatbot(args.model_path, args.adapter_path)
    chatbot.load_model()
