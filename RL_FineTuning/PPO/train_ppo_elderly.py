...
class ScriptArguments:
    ...
    ppo_adapter_output_dir: str = field(default="./output/elderly/ppo_adapter_elderly", metadata={"help": "PPO LoRA适配器保存目录"})
    ...
    # 设备配置
    policy_device: str = field(default="auto", metadata={"help": "策略模型和参考模型所在的设备"})
    reward_device: str = field(default="auto", metadata={"help": "奖励模型和价值模型所在的设备"})
...
def setup_swanlab(args: ScriptArguments):
    if not args.use_swanlab:
        return
    os.environ["SWANLAB_PROJECT"] = "qwen3-sft-rm-ppo-elderly"
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    swanlab.init(
        project="qwen3-sft-rm-ppo-elderly",
        run_name="ppo-training-elderly",
        config=vars(args)
    )
...
def load_prompts(dataset_path, tokenizer, system_prompt):
    ...
    for item in data:
        ...
        system_text = item.get('system', system_prompt)
        ...
        if human_input:
            formatted_prompt = tokenizer.apply_chat_template(
                [{"role": "system", "content": system_text}, {"role": "user", "content": human_input}],
...
def main():
    ...
    ppo_config = PPOConfig(
        ...
        num_ppo_epochs=4, output_dir="./output/elderly/ppo_model_temp_elderly",
    ...
    reward_device = "cuda:0" if torch.cuda.is_available() else "cpu"
    
    # 如果用户指定了设备且不是"auto"，则使用用户指定的设备
    ...
    if policy_device.startswith("cuda"):
        device_index = int(policy_device.split(":")[1]) if ":" in policy_device else 0
        if device_index >= torch.cuda.device_count():
            print(f"⚠️ 警告: 策略模型指定的设备 {policy_device} 不可用，使用默认设备 {policy_device}")
            policy_device = "cuda:0" if torch.cuda.is_available() else "cpu"
            
    if reward_device.startswith("cuda"):
        device_index = int(reward_device.split(":")[1]) if ":" in reward_device else 0
        if device_index >= torch.cuda.device_count():
            print(f"⚠️ 警告: 奖励模型指定的设备 {reward_device} 不可用，使用默认设备 {reward_device}")
            reward_device = "cuda:0" if torch.cuda.is_available() else "cpu"

    print(f"🔧 策略模型将使用设备: {policy_device}")
    print(f"🔧 奖励模型将使用设备: {reward_device}")
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.model_path, torch_dtype=torch.bfloat16, trust_remote_code=True, device_map=policy_device
    )
    ...
    rm_model_base = AutoModelForSequenceClassification.from_pretrained(
        args.model_path, num_labels=1, torch_dtype=torch.bfloat16, trust_remote_code=True, device_map=reward_device
    )
    ...
    ppo_trainer = PPOTrainer(
        ...
        processing_class=tokenizer, train_dataset=train_dataset, eval_dataset=eval_dataset,
