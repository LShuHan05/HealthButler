...
# #     processed_data = []
# #     for item in data:
# #         # 确保 chosen 和 rejected 是字符串
# #         chosen_response = item['chosen']
# #         rejected_response = item['rejected']
        
# #         # 如果是字典，提取 value 字段
# #         if isinstance(chosen_response, dict) and 'value' in chosen_response:
# #             chosen_response = chosen_response['value']
# #         if isinstance(rejected_response, dict) and 'value' in rejected_response:
# #             rejected_response = rejected_response['value']
        
# #         # 确保是字符串类型
# #         chosen_response = str(chosen_response) if chosen_response else ""
# #         rejected_response = str(rejected_response) if rejected_response else ""
        
# #         # 获取 human input - 简化处理，直接使用第一个 human 对话
# #         human_input = ""
...
# #         if human_input and chosen_response.strip() and rejected_response.strip():
# #             processed_data.append({
# #                 "input": human_input,
# #                 "chosen": chosen_response,
# #                 "rejected": rejected_response
...
# #     # 验证处理结果
# #     if processed_data:
# #         print(f"示例数据 - chosen 类型: {type(processed_data[0]['chosen'])}, 内容: {repr(processed_data[0]['chosen'][:50])}...")
# #         print(f"示例数据 - rejected 类型: {type(processed_data[0]['rejected'])}, 内容: {repr(processed_data[0]['rejected'][:50])}...")
...
def load_rm_dataset(dataset_path):
    ...
    processed_data = []
    for item in data:
        # RM数据集直接包含input、chosen和rejected字段
...
def main():
    ...
    print("🚀 3. 加载和预处理数据集...")
    raw_data = load_rm_dataset(args.dataset_path)  # 使用新的加载函数
    full_dataset = Dataset.from_list(raw_data)
    train_test_split = full_dataset.train_test_split(test_size=0.1)
    train_dataset = train_test_split['train'].map(preprocess_function, batched=True)
    eval_dataset = train_test_split['test'].map(preprocess_function, batched=True)
